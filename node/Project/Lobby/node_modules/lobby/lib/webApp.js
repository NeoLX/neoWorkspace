...
function WebApp(app, path){
    ...
    this.init = function (app, path) {
        ...
        fs.readFile(path + "/web.json", 'utf8', function (err, data) {
    ...
    this.setStatus = function (status) {
        ...
        if(_status != status){
            if((_status == WebApp.Status.WebConfigReady
                && status == WebApp.Status.DefConfigReady)
                || (status == WebApp.Status.WebConfigReady
                && _status == WebApp.Status.DefConfigReady)){
                    _status = WebApp.Status.Inited;
            }else{
            ...
            this.emit(WebApp.Handles.onStatusChange);
        }
    }
